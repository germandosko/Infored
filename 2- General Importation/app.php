...
$queryFindProviders = "SELECT * FROM tmp_cancer_stanfordhospital INNER JOIN tmp_cancer_stanford_result ON tmp_cancer_stanfordhospital.record_number = tmp_cancer_stanford_result.record_id AND tmp_cancer_stanfordhospital.medical_education <> 'N/A'";
...
*/
function run(){
	createInsertEducation();
}
function createInsertResidency(){
	global $queryFindProviders;
	$search = new Search();
	$clean = new Clean();
	$arrayProviders = $search->query($queryFindProviders);	
	...
	for ($i=0; $i < $numProperties; $i++) { 
			$id = $i+1;
			if($arrayProviders[$i]['Residency_2'] <> 'N/A'){
